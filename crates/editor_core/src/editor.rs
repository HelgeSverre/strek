...
impl Editor {
    ...
    pub fn toggle_selected_frame_background(&mut self) -> bool {
        let background = self
            ...
            .map_or_else(|| Some(Paint::white()), |_| None);
        self.set_selected_frame_background_with_description(background, "Toggle Frame Background")
    }

    /// Set the selected frame background while preserving undo and redo history.
    pub fn set_selected_frame_background(&mut self, background: Option<Paint>) -> bool {
        self.set_selected_frame_background_with_description(background, "Set Frame Background")
    }

    fn set_selected_frame_background_with_description(
        &mut self,
        ...
        let command = Command::new(description).with_patch(Patch::SetFrame { id, before, after });
        command.apply(&mut self.document);
        self.history.push(command);
        self.needs_redraw = true;
        true
...
mod tests {
    ...
    #[test]
    fn frame_background_color_is_undoable() {
        let mut editor = Editor::new();
        let frame = editor
            .document
            .add_child(editor.document.root, Node::frame("Frame", 100.0, 100.0))
            .unwrap();
        editor.selection.set([frame]);
        let background = Paint::rgba(0.2, 0.4, 0.8, 0.75);

        assert!(editor.set_selected_frame_background(Some(background.clone())));
        assert_eq!(
            editor.selected_frame_data().unwrap().background,
            Some(background)
        );
        assert_eq!(
            editor.history.undo_description(),
            Some("Set Frame Background")
        );
    ...
    #[test]
    fn frame_creation_endpoint_snaps_to_object_geometry() {
        let mut editor = Editor::new();
        editor
            .document
            .add_child(
